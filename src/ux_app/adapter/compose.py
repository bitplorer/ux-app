"""Fold author items into one Channel ``ops[]``.

Accepts wire dicts, ``ui.dom.morph`` Ops, and motion Scenes (``.play()`` /
``.ops()``). Does not import ``ux_channel`` or ``ux_motion``.

Law on one Result: ``morph(T)`` XOR ``scene.enter(T, html=…)``.
Both would write the same node — Channel idiomorph then motion injectHtml —
and remount images. Reject at compose time.

Navigate kinds are ordered last so a morph is not abandoned mid-apply.
"""
...
_NAV_KINDS = frozenset({"navigate", "push_url", "reload"})
_MORPH_KINDS = frozenset({"morph", "swap"})


class ComposeConflict(ValueError):
    ...
    if isinstance(item, Op):
        if item.pair != ("ui.dom", "morph"):
            raise TypeError(
                ...
                "Pass a Channel wire dict or a Scene."
            )
        return [lower_morph(item.payload.get("target") or "", item.payload.get("patch") or "")]
...
def _order(ops: list[dict[str, Any]]) -> list[dict[str, Any]]:
    head = [op for op in ops if op.get("op") not in _NAV_KINDS]
    tail = [op for op in ops if op.get("op") in _NAV_KINDS]
    return head + tail
...
def _reject_overlap(ops: list[dict[str, Any]]) -> None:
    ...
    for op in ops:
        ...
        if kind in _MORPH_KINDS and op.get("target"):
