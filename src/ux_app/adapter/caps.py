"""Capability mint / verify for the in-process runtime.

When Channel is present, boot may wrap its CapService instead.
This module does not import ux_channel at module level.

Tokens are ``base64url(json).sig`` so action names may contain dots
(``cart.badge.add``). A dotted ``split(".")`` encoding is illegal here.
"""
...
class OnceStore:
    ...
    def consume(self, cap_id: str) -> None:
        if self.down:
            raise CapError("once-store down")
        if cap_id in self._seen:
            raise CapError("cap already consumed")
        self._seen.add(cap_id)

    def seen(self, cap_id: str) -> bool:
        return cap_id in self._seen
...
class CapMachine:
    def __init__(self, secret: str | None = None, *, max_age: int = 3600) -> None:
        self.secret = (
            secret or os.environ.get("UX_APP_SECRET") or "dev-secret-key-32chars-minimum!!!!"
        ).encode("utf-8")
    ...
    def verify(self, token: str | None, action: str, args: dict[str, Any]) -> dict[str, Any]:
        if not token:
        ...
        try:
            ...
            raise CapError("cap expiry") from exc
